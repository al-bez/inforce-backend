...
import { IUser, TUserRole, IRequestWithAdditionalFields } from 'types'
...
const createSendToken = (user: IUser, statusCode: number, res: Response) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() + parseInt(env.JWT_COOKIE_EXPIRES_IN) * 24 * 60 * 60 * 1000
    ),
    httpOnly: true,
...
const protect = catchAsync(
  async (req: IRequestWithAdditionalFields, res, next) => {
...
const restrictTo = (...roles: TUserRole[]) => {
  return (
